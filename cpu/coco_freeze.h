...
struct Coco_Cell {
    ID_TYPE src;
    ID_TYPE dst;
    DATA_TYPE coco_count;
    DATA_TYPE pure_count;
    //DATA_TYPE freeze_count;
};

template<typename ID_TYPE, typename DATA_TYPE>
class Coco_TCM_freeze {
public:
    Coco_TCM_freeze() {}
    Coco_TCM_freeze(uint32_t memory, double rate){
        tcm_sketch = new TowerTCM_v2<ID_TYPE, DATA_TYPE> (memory * (1 - rate));
        array_n = 2;
        d = sqrt(memory * rate * 1024 * 0.97 / (sizeof(Coco_Cell<ID_TYPE, DATA_TYPE>)) / array_n);
    ...
    void insert(ID_TYPE src, ID_TYPE dst, uint32_t weight) {
        ...
        if (x < p) { // evict
            ...
            tcm_sketch->insert(SRC, DST, counter[min_index][src_index][dst_index].pure_count, true);
            tcm_sketch->freeze(src, dst);
            counter[min_index][src_index][dst_index].pure_count = weight;
        } else {
            tcm_sketch->insert(src, dst, weight, false);
        }
    }
...
private:
    ...
    TowerTCM_v2<ID_TYPE, DATA_TYPE>* tcm_sketch;
};
