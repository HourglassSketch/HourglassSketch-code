...
public class GraphDatabaseFacade implements GraphDatabaseAPI
{
            ...
            long timeoutMillis, Consumer<Status> terminationCallback, TransactionExceptionMapper transactionExceptionMapper )
    {
        ...
        loadWCSFromFile(config.get(neo4j_home).getFileName().toString(),transaction);
        return transaction;
    }
    public void loadWCSFromFile(String name, TransactionImpl transaction){
        System.out.println("Create Hourglass: ");
        ...
        // if(!file.exists()) {
            ...
            try {
                file.createNewFile();
                transaction.WBSketch = new WCSketch(100, file.getPath());
                transaction.HourglassSketch = new HourglassSketch(10000);
                System.out.println("Create Hourglass finished");
            } catch (Exception e){
                e.printStackTrace();
                //log.error(e.getMessage());
            }
        // } else {
        //     transaction.WBSketch = WCSketch.loadFromFile(file);
        // }
    }
    @Override
